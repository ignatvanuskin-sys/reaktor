...
function BookingForm({ onClose }: { onClose?: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/booking", {
        ...
        body: JSON.stringify({ ...form, source: "site-booking" }),
      });
  ...
  return <div className="booking-card">
    ...
    {step === 2 && <div className="form-grid single"><label>Что нужно сделать<select id="booking-service" value={form.service} onChange={(e) => update("service", e.target.value)} {...field("service")}><option value="">Выберите услугу</option>{["Диагностика", "Ремонт ходовой части", "Ремонт двигателя", "Промывка форсунок", "Замена масла", "Ремонт сцепления", "Электрика", "Подбор запчастей", "Другое"].map((s) => <option key={s}>{s}</option>)}</select>{fieldErrors.service && <small className="field-error" id="booking-service-error">{fieldErrors.service}</small>}</label><label>Опишите проблему <em>необязательно</em><textarea rows={3} placeholder="Например: стук в передней подвеске при поворотах" value={form.problem} onChange={(e) => update("problem", e.target.value)} /><small>Можно описать симптом — мастер уточнит детали при связи.</small></label></div>}
...
export default function Home() {
  const [bookingOpen, setBookingOpen] = useState(false); const [menuOpen, setMenuOpen] = useState(false); const [showFloatingBook, setShowFloatingBook] = useState(false);
  useEffect(() => {
